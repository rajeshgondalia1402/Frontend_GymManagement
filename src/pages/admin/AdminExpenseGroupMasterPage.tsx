...
export function AdminExpenseGroupMasterPage() {
  ...
  const createMutation = useMutation({
    mutationFn: adminService.createExpenseGroup,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-expense-groups'] });
      setDialogOpen(false);
      reset();
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { expenseGroupName: string } }) => 
      adminService.updateExpenseGroup(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-expense-groups'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: adminService.deleteExpenseGroup,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-expense-groups'] });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage admin expense group records</p>
        </div>
...
export default AdminExpenseGroupMasterPage;
